...
public:
  ...
  BookcodeCreatorStrategyRandom(int numOfPositions, int windowSize)
      : numOfPositions(numOfPositions), windowSize(windowSize){};
  std::tuple<codebook_type, vectors_type> make(Image &image);
  bool isInCodebook(std::vector<std::vector<int>> &codebook,
