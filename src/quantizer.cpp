...
Image Quantizer::quantize(Image &image,
  ...
  Image outputImage =
      imageFromVectors(outputVectors, image, bookcodeCreator.windowSize);
  return outputImage;
}

double Quantizer::distanceBetweenVectors(std::vector<int> vec1,
                                         std::vector<int> vec2) {
  double sum = 0;
...
Image Quantizer::imageFromVectors(std::vector<std::vector<int>> &vectors,
                                  Image &originalImage, int windowSize) {
      ...
      for (int j = 0; j < vectorsPerWidth; j++) {
        for (int l = 0; l < windowSize; l++) {
          flat_image.push_back(
              vectors.at(i * vectorsPerWidth + j).at(k * windowSize + l));
        }
      }
    }
  }
